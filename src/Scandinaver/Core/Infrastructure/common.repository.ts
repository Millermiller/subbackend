...
export class CommonRepository<D extends Entity> {
  ...
  public async one(id: number): Promise<D> {
    return this.api.one(id).then(response => plainToClass(this.type, response.data))
  }
  ...
  public async update(entity: D, data: any): Promise<D> {
    return this.api.update(entity, data).then(response => plainToClass(this.type, response.data))
  }

  public async delete(entity: D): Promise<any> {
    return this.api.delete(entity).then(response => plainToClass(this.type, response.data))
  }
}
