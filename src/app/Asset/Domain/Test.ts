...
import Question from '@/app/Asset/Domain/Question'
...
export class Test extends Entity {
  ...
  constructor(asset: Asset) {
    super()
    this._asset = asset
    const translates = asset.cards.all().map(card => card.translate)
    const questions = this._asset.cards.all().map(card => new Question(card, translates))
