<template lang="pug">
  li.columns
    p(:class="['column', 'is-7', 'pointer', {'info': item.id === activeAssetId}]", @click="load(item.id)") {{item.level}}. {{item.title}}
      span.text-success ({{item.cards_count}})

    p.column.control-block.is-5
      a.button.is-small(@click="$emit('edit', item)")
        span.icon
          i.fa.fa-pencil

      a.button.is-small(@click="load(item.id)")
        span.icon
          i.fa.fa-eye

      a(:class="['button', 'is-small', {'is-loading': loaded }]", @click="forvo(item.id)") Forvo
